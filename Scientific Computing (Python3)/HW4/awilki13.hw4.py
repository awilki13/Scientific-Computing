...
def Ax(v):
  ...
  # use the matrix provided for coefficients and vector locations to reference them
  Ax[0] = -4 * v[0] + v[1] + v[3]
...
x = zeros((9))*1.0
tol = 1e-06
# simply uncomment and plug in the Ax function to the conjGrad function with an empty x array and the given b array.
...
# for different tolerances plug in the Ax vector with an empty x array and the given b array
